...
import { loadRules, formatRulesForPrompt, getRulesSummary, collectCodebaseContext, generateProjectRules } from "../src/rules.js";
...
test("Project Rules Engine", async (t) => {
  ...
  await t.test("collectCodebaseContext discovers directories, files, and key configs", async () => {
    ...
    await fs.writeFile(path.join(appDir, "src", "index.ts"), "export const a = 1;");

    const context = await collectCodebaseContext(appDir);
    assert.ok(context.tree.some((t) => t.includes("package.json")));
    assert.ok(context.tree.some((t) => t.includes("src")));
    assert.ok(context.configs["package.json"].includes("my-real-app"));
  });

  await t.test("generateProjectRules generates factual, professional FIXY.md file", async () => {
    const appDir = path.join(tmpDir, "sample-app");
    const generated = await generateProjectRules({ format: "FIXY.md" }, appDir);
    assert.strictEqual(generated.filename, "FIXY.md");
    assert.ok(generated.content.includes("my-real-app"));
    assert.ok(generated.content.includes("vite build"));
